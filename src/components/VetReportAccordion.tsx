...
export function VetReportAccordion({ session, pet }: VetReportAccordionProps) {
  ...
  const rows: [string, string][] = [
    ['Tier', `${petName} - ${pet.species === 'hund' ? 'Hund' : 'Katze'} - ${pet.ageYears} J. - ${pet.weightKg} kg`],
    ...
    <div style={{ border: `1px solid ${T.border}`, borderRadius: 13, overflow: 'hidden' }}>
      <button
        style={{
          ...
          padding: '14px 16px', background: T.pLight, border: 'none', cursor: 'pointer',
          fontSize: 13, fontWeight: 700, color: T.primary, fontFamily: 'inherit',
        }}
        onClick={() => setOpen(v => !v)}
        aria-expanded={open}
      >
